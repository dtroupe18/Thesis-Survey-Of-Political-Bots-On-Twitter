...
class BotometerClient:
    ...
    def start_bot_collection(self):
        ...
        self.df['user_id'] = self.df['user_id'].astype('int64')
        ...
        for index, row in self.df.iterrows():
            ...
            try:
                ...
                if cap > 0.70:
                    ...
                    self.send_still_active_tweet(payload['user']['screen_name'], cap)
    ...
    def send_still_active_tweet(self, user, cap):
        cap *= 100
        cap = round(cap, 2)

        if cap < 90:
            start = 'It looks like {0} is still active on Twitter '.format(user)
            ending = ' despite having a botometer score of {0}% #politicalbots'.format(cap)
        else:
            start = 'Twitter still has\'t detected that {0} is an obvious bot,'.format(user)
            ending = ' despite having a botometer score of {0}% #politicalbots'.format(cap)
    ...
    def load_new_ids_df(self):
        path = os.path.dirname(os.path.abspath(__file__)) + '/' + self.ids_file_name
